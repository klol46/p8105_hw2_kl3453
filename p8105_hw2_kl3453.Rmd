...
# Problem 2 (This was initially done with old dataset)
...
__Response__
  mr_trash_df contains the converted xlsx file to R studio dataframe data which has 584 observations of which are the days that the trash wheel was deployed and used. The amount of trash collected was enumerated (ie. weight_tons) and the types of trash collected where enumerated as well (ie. plastic_bottles) and a final "houses_powered" variable results as a way to measure the amount of trash collected. prof_trash_df and gwynna_trash_df are similar but contain 106 observations and 155 observations respectively. The final combination of the 3 contains the sum of the three separate dataframes as the dataframes are stacked together into one dataframe.
...
gwynnda_trash_df_1 =
  ...
  filter(year == 2021)
sum(pull(gwynnda_trash_df_1,cigarette_butts))
```

__Response__
  The total # of cigarette butts collected by Gwynnda in July of 2021 is 16,300 butts.
